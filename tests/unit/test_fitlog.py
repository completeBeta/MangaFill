...
def logging_on(tmp_path, monkeypatch):
    ...
    # The accumulator is module-global, so it must be cleared or one test's
    # records leak into the next test's summary.
    fitlog.reset()
...
def test_summary_line_reaches_the_app_log(logging_on, caplog):
    ...
    import logging
    # Reset immediately before recording: the accumulator is module-global, so any
    # render another test left behind would otherwise appear in this summary.
    fitlog.reset()
    ...
    assert "degenerate=0" in line and "unmeasurable=0" in line
