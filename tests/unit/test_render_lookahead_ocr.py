...
def test_merge_keeps_the_engine_translations():
    """HYBRID (v0.30.5): merging must not throw `translation` away.

    Measured on the Korean test page 001: the engine's lower-plate region and the second
    pass's recovered box for that SAME plate resolve to one bubble, and the merge used to
    rebuild the block with `text` only — so the region loop skipped it for having nothing to
    draw, the plate came back untouched (Korean still visible, no English), and the job read
    "2 blocks · 1 translated". That was the defect behind every failed second-pass form, and
    it was never in second_pass.py.
    """
    bubbles = [(0, 400, 500, 1400)]  # (x, y, w, h) — must contain BOTH boxes (y 657 and 1534)
    a = TextBlock(bbox=(145, 1534, 253, 38), text="대한민국매출규모", confidence=0.9,
                  translation="South Korea sales scale")
    b = TextBlock(bbox=(139, 657, 141, 117), text="1위를 다리고 이는", confidence=0.9,
                  translation="is competing for first place")
    out = render._merge_blocks_per_bubble([a, b], bubbles)
    assert len(out) == 1
    # members are merged in reading order (top-to-bottom), so the UPPER box's text comes first
    assert out[0].text == "1위를 다리고 이는 대한민국매출규모"
    assert out[0].translation == "is competing for first place South Korea sales scale"


def test_merge_of_untranslated_blocks_still_has_no_translation():
    """The OCR path must be unchanged: those blocks are translated AFTER this merge."""
    bubbles = [(0, 400, 320, 400)]
    a = TextBlock(bbox=(1, 455, 311, 268), text="L선배!선배!")
    b = TextBlock(bbox=(139, 659, 141, 117), text="내말들려요?")
    out = render._merge_blocks_per_bubble([a, b], bubbles)
