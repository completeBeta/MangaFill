...
def test_translate_page_retries_missing_lines(monkeypatch):
    def fake_post(*a, **k):
        # the real prompt is the LAST message; earlier ones carry upstream's few-shot example
...
def test_system_prompt_korean_uses_manhwa_variant():
    ...
    # the generic prompt must NOT carry the Korean register section
    assert "Korean register" not in translate._system_prompt("ja")


def test_system_prompt_other_langs_use_generic_prompt():
    ja = translate._system_prompt("ja")
    zh = translate._system_prompt("zh")
    assert "The text to translate is Japanese" in ja
    assert "The text to translate is Chinese" in zh
    assert "banmal" not in ja
    assert ja != translate._system_prompt("ko")
    # the TARGET is English in every case (the source language is named separately)
    for src in ("ja", "zh", "ko"):
        assert "sound natural in English" in translate._system_prompt(src)
