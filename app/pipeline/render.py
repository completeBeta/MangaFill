...
from .captions import recover_captions
from . import fitlog
from . import replay
from . import balloon_local
...
def _warn(msg: str, *args) -> None:
    """Log through the app's logger. Diagnostics must never break a render."""
    try:
...
def _say(msg: str, *args) -> None:
    """Same as `_warn` but at INFO — for decisions that changed a page (merges, drops).

    A change to a page that only reaches stdout is invisible in the webui log viewer,
    which is how the bubble-merge translation loss stayed hidden through three releases.
    """
    try:
        from app.services.logging import get_logger
...
def _punctuation_only(text: str) -> bool:
    """True when a block's reading carries no letters or digits at all.

    A lone `。` became a block, was translated to ".", and was lettered as a one-glyph
    caption (job-3 page 9, box 41x21). Nothing of value is lost by leaving punctuation-only
    boxes alone — the mark is already on the page — and it removes a class of junk blocks
    that only exist because the OCR found a stray mark.
    """
...
def _merge_blocks_per_bubble(blocks: list, bubbles: list) -> list:
    ...
    for b in blocks:
        ...
        # HYBRID (v0.30.5) — CARRY THE TRANSLATIONS, or the page is silently lost. This helper
        # was written for the OCR path, where blocks arrive with `text` only and the LLM fills
        # `translation` in LATER, so rebuilding the block without a translation cost nothing.
        # Engine (hybrid) blocks arrive ALREADY translated, so any merge used to throw the
        # translation away: the engine's lower-plate region and the second-pass box for that
        # same plate resolve to ONE bubble, merged into a block holding only the joined SOURCE
        # text, and the region loop then skipped it for having nothing to draw — the plate came
        # back untouched (Korean still visible, no English) while the job read "2 blocks · 1
        # translated". That was the defect behind EVERY failed second-pass form (v0.30.1-.3); it
        # was never in second_pass.py. Members are translated once each by the engine, so join
        # the translations in the same reading order as the text rather than re-translating.
        translation = " ".join(m.translation for m in members
                               if getattr(m, "translation", ""))
        _say("merged %d blocks in one bubble -> text=%r translation=%r",
             len(members), text[:60], translation[:60])
...
def _recover_thin_region(gray_page, box, region, text, page_w, page_h, obstacles):
    """Class C recovery: a usable lettering region for a block whose region is
    far too small for its text.

    Large hand-lettered (brush) lines get detected as thin strips. Measured on job-3
    page 163: box [702,457,197,39] for まったく上がらす！, and region resolution then
    squeezed the region to 197x8 — so the English was lettered at 8px over a large
    blank area (the source text is erased correctly, so the page looks empty).

    The honest region is "at least what we erased": union the block's own erase rects
    and use that when it is bigger than the region and still sane.
    """
    rw, rh = int(region[2]), int(region[3])
    if min(rw, rh) >= 14 or len(text or "") < 8:
        return region
    try:
        rects = _erase_rects(gray_page, box)
    except Exception:
        return region
    if not rects:
        return region
    x0 = max(0, min(int(r[0]) for r in rects))
    y0 = max(0, min(int(r[1]) for r in rects))
    x1 = min(page_w, max(int(r[0]) + int(r[2]) for r in rects))
    y1 = min(page_h, max(int(r[1]) + int(r[3]) for r in rects))
    cw, ch = max(1, x1 - x0), max(1, y1 - y0)
    # Growth is measured against the BLOCK's box, not the squeezed region: the case we
    # are fixing is exactly the one where the region is pathologically small (197x8 for a
    # 197x39 line), so capping against the region would reject the real fix. Measured
    # p163: 300x80 footprint = 3.1x the block box, comfortably inside 8x.
    base = max(rw * rh, int(box[2]) * int(box[3]), 1)
    if cw * ch <= rw * rh:
        return region
    if cw * ch > 8 * base or cw * ch > 0.06 * page_w * page_h:
        return region
    for ob in obstacles:
        if tuple(ob) == tuple(box):
            continue
        if (ob[0] >= x0 and ob[1] >= y0
                and ob[0] + ob[2] <= x0 + cw and ob[1] + ob[3] <= y0 + ch):
            return region
    return (x0, y0, cw, ch)
...
def _free_text_region(bbox: tuple, page_w: int, page_h: int, gray=None,
    ...
    """
    # 1.3, not 1.5: a 50x75 single-word box (job-3 p10's "Flinch") missed the old threshold
    # by a pixel and kept its 9px column. The widening is bounded by siblings, panel rules
    # and a 2.5x cap, so a slightly lower bar for "this is a column, not a label" is safe.
    if bbox[3] > bbox[2] * 1.3:
...
# --- class D: a region that lies mostly OFF the page (job-2 p56) -----------------------
# A stored box can lie mostly outside its page: p56's caption has y+h = 1681 on a 1600px
# page, so only 36 of its 117 rows are on-page. The erase ran (the Korean went) and the
# English was lettered into the off-page part — the page came back with a black band and no
# translation. Clamping the region to the page keeps the lettering ON the page; a region
# with almost nothing on-page is dropped entirely (and logged) rather than lettered into a
# sliver a reader cannot see.
_REGION_MIN_ON_PAGE = 12


def _region_on_page(region, page_w: int, page_h: int, min_side: int = _REGION_MIN_ON_PAGE):
    """`region` clamped to the page, or None when too little of it is on-page."""
    try:
        x, y, w, h = [int(v) for v in region]
    except Exception:
        return None
    x0, y0 = max(0, x), max(0, y)
    x1, y1 = min(int(page_w), x + w), min(int(page_h), y + h)
    if x1 - x0 < min_side or y1 - y0 < min_side:
        return None
    return (x0, y0, x1 - x0, y1 - y0)


def _erase_rects_checked(gray, box, page_w: int, page_h: int) -> list[tuple]:
    """`_erase_rects` for a box with ON-PAGE area; nothing at all when it has none.

    Never erase text we cannot then letter: the erase is irreversible, so a block whose
    box is off-page keeps its source text and its translation is dropped (logged) instead
    of leaving a hole.
    """
    if _region_on_page(box, page_w, page_h) is None:
        return []
    return _erase_rects(gray, box)
...
def _own_balloon_region(orig_bgr, block, region, siblings, page_w: int, page_h: int):
    """This block's OWN balloon interior when `region` is a MERGED one; else None.

    `region` is a shared interior whenever it swallows a SIBLING's source box — that
    is the two-balloon class (591 regions / 1,329 blocks on job 3 of the v0.27.41
    render; 1,199 blocks flagged `merged`). The local window in
    `balloon_local.local_balloon_region` is what tells the two balloons apart: it asks
    a question about THIS block's neighbourhood only, so a neighbouring outline inside
    the window acts as a wall instead of being merged with ours.

    Accepted only when the derived balloon still answers the question that matters —
    it must NOT swallow a sibling's box (>50% of that box, the same bar `_sibling_boxes`
    uses to call a block "nested"). Otherwise we keep the region we had and the caller's
    sibling clip gets its chance; an extraction that re-merges the pair buys nothing.

    Returns `(mask, box)` in page coordinates, or None.
    """
    if orig_bgr is None or region is None or not siblings:
        return None
    if not balloon_local.ENABLED:
        # Ported, then measured on all three languages: neutral overall (ja -3.2% outside
        # ink, ko -0.2%, zh +0.4%) with `narrow` worse in all three. Off by default until
        # the tightened form (clip, don't replace) shows a win — see the switch note in
        # `balloon_local`.
        return None
    if not _overlaps_any(region, siblings):
        return None                     # unambiguous already — keep the resolved region
    got = balloon_local.local_balloon_region(orig_bgr, block.bbox, page_w, page_h,
                                             sibling_boxes=siblings)
    if got is None:
        return None
    mask, box = got
    if _overlaps_any(box, siblings):
        # still covering a neighbour: either the window is too small for this geometry
        # or the two balloons genuinely share one outline. Not our case to fix here.
        return None
    return mask, box
...
def _clip_region_off_siblings(region, block_box, siblings) -> tuple | None:
    """Shrink a region that reaches past a SIBLING block's own source box.

    TWO OVERLAPPING BALLOONS FLOOD AS ONE INTERIOR. Measured on job-3 page 10: the
    detector/flood resolved a 427x445 region shared by three blocks, and a 313x355 one
    shared by two, so each block's lettering was fitted as a wide rectangle spanning BOTH
    balloons — the reader sees English drawn straight across the two outlines, and in the
    same box the type is sized for the union, not for its own balloon ("CO! CO..." at 23px
    over artwork that belongs to the neighbouring balloon; "How far can you go?" using 22%
    of the width it was handed).

    The SOURCE TEXT'S OWN BOX is the anchor: a block may use the balloon around it, never
    the neighbour. Each side of the region is pulled in to the first sibling box that
    overshoots it, and never inside the block's own box. Returns the clipped region, or
    None when nothing changes (so the caller's behaviour is untouched everywhere else).
    """
    rx, ry, rw, rh = (int(v) for v in region)
    bx, by, bw, bh = (int(v) for v in block_box)
    x0, y0, x1, y1 = rx, ry, rx + rw, ry + rh
    cx, cy = bx + bw / 2.0, by + bh / 2.0
    for s in siblings or ():
        sx, sy, sw, sh = (int(v) for v in s)
        if sw <= 0 or sh <= 0:
            continue
        sx1, sy1 = sx + sw, sy + sh
        if min(y1, sy1) > max(y0, sy):          # the sibling shares our rows
            if sx1 <= cx and sx1 > x0:
                x0 = sx1                        # it sits on our left
            elif sx >= cx and sx < x1:
                x1 = sx                         # ...on our right
        if min(x1, sx1) > max(x0, sx):          # the sibling shares our columns
            if sy1 <= cy and sy1 > y0:
                y0 = sy1                        # ...above us
            elif sy >= cy and sy < y1:
                y1 = sy                         # ...below us
    x0, y0 = min(x0, bx), min(y0, by)
    x1, y1 = max(x1, bx + bw), max(y1, by + bh)
    clipped = (x0, y0, max(1, x1 - x0), max(1, y1 - y0))
    return None if clipped == (rx, ry, rw, rh) else clipped
...
def _caption_blocked(gray, obstacles=None, gap: int = _CAPTION_OBSTACLE_GAP,
                     ink: bool = True) -> np.ndarray:
    ...
    `ink=False` drops the artwork and keeps only the OTHER TEXT blocks — the mask for case
    E below (narration over artwork), where the artwork must not veto the width.
    """
    blocked = np.asarray(gray) < _CAPTION_INK if ink else np.zeros_like(np.asarray(gray), dtype=bool)
    H, W = blocked.shape[:2]
...
def _thin_walls(gray, y0: int, y1: int, min_frac: float = 0.75,
                max_thick: int = 14) -> np.ndarray:
    """Columns the lettering must not be widened across: THIN, TALL lines of ink.

    Class E widens narration across artwork, because the source text was inked over that
    artwork too. A panel RULE is different: a line of English drawn across a panel border
    looks broken, and the source never crossed it. Both are "ink", so thickness is what
    separates them — a rule is a run of a few inked columns, artwork is a broad mass. Only
    runs up to `max_thick` wide, inked through most of the strip's height, count as walls.
    """
    ink = np.asarray(gray) < _CAPTION_INK
    H = ink.shape[0]
    y0, y1 = max(0, int(y0)), min(H, int(y1))
    if y1 - y0 < 8:
        return np.zeros(ink.shape[1], dtype=bool)
    # CONTIGUOUS run, not coverage: a panel rule is inked solidly through the whole strip,
    # while artwork (a character's hair with its highlight streak) is interrupted — measuring
    # coverage classified the hair as a wall and killed the widening it exists for.
    _runs = _max_run(ink[y0:y1])
    tall = _runs >= max(8, int(min_frac * (y1 - y0)))
    walls = np.zeros(ink.shape[1], dtype=bool)
    c, W = 0, tall.shape[0]
    while c < W:
        if tall[c]:
            s = c
            while c < W and tall[c]:
                c += 1
            if c - s <= max_thick:
                walls[s:c] = True
        else:
...
def _caption_region(bbox: tuple, page_w: int, page_h: int, gray=None,
    ...
    # --- class E: narration drawn OVER ARTWORK -----------------------------------------
    # The source text itself was inked on top of the art (job-3 page 10: a 54x346 column of
    # narration lies across the character's hair), so artwork must not veto the width — only
    # other TEXT must. Without this the strip cannot grow at all (both sides are ink), the
    # English is lettered into 54px, and the fit collapses to 8px: "I nearly died a few
    # times, though..." came out as seven 8px lines beside dialogue lettered at 22px. The
    # original lettering had exactly the same problem to solve and solved it by drawing
    # wider text over the art; our lettering carries its own outline, so it stays legible
    # there. Bounded by siblings, the page, and the same 2.5x cap as the free-space path.
    if gx == 0 and max_x > 0:
        _sib = _caption_blocked(gray, obstacles, ink=False)
        # ... but never across a thin, tall ink line: that is a panel rule, and the source
        # text never crossed it (the caption-region bounds test pins exactly this fixture).
        _walls = _thin_walls(gray, y, y + h)
        if _walls.any():
            _sib[:, _walls] = True
        _allowed = _free_lines(_sib, y, y + h, 0)
        # ASYMMETRIC on purpose. The obstruction is often on ONE side only — job-3 p10's
        # column sits 4px from its panel's left border with open artwork to the right — and
        # the symmetric `_run_room` capped the growth at that 4px, which is why the first
        # version of this fix did nothing on the very block it was written for.
        _L = _R = 0
        while _L < max_x and x - _L - 1 >= 0 and _allowed[x - _L - 1]:
            _L += 1
        while _R < max_x and x + w + _R < _allowed.shape[0] and _allowed[x + w + _R]:
            _R += 1
        if _L + _R > 0:
            _warn("narration-over-art: widened %dpx -> %dpx over artwork (box %s)",
                  w, w + _L + _R, (int(x), int(y), int(w), int(h)))
            _nx = max(0, min(x - _L, page_w - (w + _L + _R)))
            return (_nx, max(0, min(y - gy, page_h - (h + 2 * gy))), w + _L + _R, h + 2 * gy)
...
def _build_blocks_from_det(det: dict, image_np: np.ndarray) -> list[TextBlock]:
    ...
    for (x, y, w, h) in _dedup_boxes(det["text_bubble"], seen, keep="largest"):
        ...
        if (not text or len(text.strip()) <= 1 or not _has_japanese(text)
            ...
            continue  # already-English / non-JP / bare-punctuation bubble — leave untouched
    ...
    for (x, y, w, h) in _dedup_boxes(free_vertical, seen, keep="largest"):
        text, conf = ocr_crop(image_np, (x, y, w, h))
        if not text or not _has_japanese(text) or _punctuation_only(text):
            continue  # watermark / page number / bare punctuation
        blocks.append(TextBlock(bbox=(x, y, w, h), text=text, confidence=conf,
...
def _blocks_from_engine_regions(regions: list[dict]) -> list[TextBlock]:
    """TextBlocks from the ENGINE's detected regions and ITS translations (v0.30.0).

    Feeds the hybrid path: the engine (upstream manga-image-translator) did detect + OCR
    + translate, and this app only letter the result — so these blocks arrive with
    `text` AND `translation` already filled, and the pipeline's own LLM is never called.

    Two deliberate choices, both to stop a bad upstream field doing damage here:
      * its `angle` is trusted only inside ±45°, the range our typesetter's `angle` is
        defined for — a wrong rotation is worse than none;
      * the orientation comes from the box's aspect ratio exactly as our own detector's
        blocks do, so the existing vertical/horizontal rules apply unchanged.
    A region with no translation, or whose translation equals its source text, is dropped:
    there is nothing to draw, and drawing the source back over itself would be worse than
    ...
    for r in regions or []:
        ...
        if w < 2 or h < 2 or not text or not translation or translation == text:
            continue
        try:
        ...
        if abs(angle) > 45:
            angle = 0.0
        blocks.append(TextBlock(bbox=(x0, y0, w, h), text=text, translation=translation,
                                confidence=r.get("prob"), orientation=_orientation(w, h),
                                angle=angle))
    blocks.sort(key=lambda b: (b.bbox[1], -b.bbox[0]))
    return blocks
...
def render_translated_page(
    ...
    caption_vlm: bool = False,
    engine_regions: list[dict] | None = None,
    ...
    HYBRID (`engine_regions`): when the caller has already had an ENGINE detect, OCR
    and translate the page, pass its regions here and this function does NO detection,
    NO OCR and NO LLM translation — it letters the engine's content with this app's own
    typesetter, which is the whole point (v0.30.0). Our container detector still runs for
    bubble geometry, because the balloon-fitting path needs it; see the comment at the
    injection point for why that is not optional.
    """
    ...
    # (Logged: a page that comes back untouched must never do so silently.)
    if _is_blank(image_np):
        _say("page skipped: blank page (no dialogue to translate)")
        return image, [], 0, 0, []
    ...
    # NOT when the HYBRID handed us engine regions: the engine READ text on this page, so the
    # page has dialogue whatever the colour test says, and skipping would silently blank a page
    # the engine had already translated (measured in prod 2026-09-25: a korean colour page came
    # back with 0 blocks and no log line). A wrong source language is the usual cause — the
    # caller now lets the engine's own answer override a guess, and this guard is the belt.
    if lang == "ja" and _is_color(image):
        if engine_regions:
            _warn("hybrid: colour page (lang=%s) but the engine read %d region(s) — "
                  "lettering it anyway", lang, len(engine_regions))
        else:
            _say("page skipped: colour page (ja) — no dialogue expected")
            return image, [], 0, 0, []
    ...
    # ---- HYBRID: the ENGINE's content, OUR lettering (v0.30.0) ---------------
    # Option 1 (2026-09-24). The engine finds the text and translates it —
    # its detector catches far more Korean than our own defaults did (measured: −44%
    # source text left on the page, 10-page corpus) — and THIS app draws the result
    # with the lettering that made v0.27.x look finished: fitted to the balloon's
    # outline and filling it, not centred in a rectangle.
    #
    # Both halves of that are load-bearing:
    #   * presetting `blocks` skips detect + OCR (the engine already did both) and the
    #     LLM (the engine already translated), because the detection section below is
    #     guarded by `if blocks is None`;
    #   * `bubbles` MUST still be populated. The balloon-fit path (`shaped`,
    #     `shape_masks`, `balloon_from_original`, the local re-derivation) only runs
    #     inside the `elif bubbles is not None:` branch — with no bubbles the same text
    #     falls through to the plain-rectangle branch, which IS the "small text sitting
    #     in a big box" failure this hybrid exists to remove. So we run our own
    #     container detector for GEOMETRY only; no text comes from it.
    engine_translated = False
    bubbles = None
    blocks = None
    if engine_regions is not None:
        blocks = _blocks_from_engine_regions(engine_regions)
        engine_translated = True
        try:
            emit("detect")
            _hy_det = detect_containers(image)
            bubbles = (_hy_det or {}).get("bubble") or []
        except Exception as _e:  # noqa: BLE001
            bubbles = []
            _warn("hybrid: container detector failed (%s) — lettering falls back to "
                  "rectangular placement for this page", _e)

    # ---- detect + OCR --------------------------------------------------------
    if lang in ("ko", "zh") and blocks is None:
        # HYBRID GUARD (v0.30.0): this branch is entered UNCONDITIONALLY for ko/zh, and it
        # REBUILDS `blocks` from our own PaddleOCR — which threw away the engine's regions
        # and their translations on every Korean/Chinese hybrid page (job 31: engine read 2
        # Korean regions, our OCR replaced them with untranslated boxes, the region loop then
        # skipped every one for having no translation, and the page came back untouched with
        # no inpaint stage and nothing logged). The ja path never showed it because it is
        # guarded by `if blocks is None` below. When the hybrid has already supplied the
        # engine's blocks, there is nothing to detect here.
    ...
    if dry_run:
        ...
        for b in blocks:
            b.translation = ""
    elif engine_translated:
        # HYBRID (v0.30.0): the engine already read and translated these blocks, so our
        # LLM is not called for this page at all — the translations arrived with them.
        # pt/ct stay 0 here; engine-side token accounting is a separate known gap.
        pt = ct = 0
    else:
        emit("translate")
        ...
        # NARROW RELAXATION of the gate above (v0.27.33): a pure-horizontal page is
        # normally left alone, but an IN-WORLD caption on it — a 【…】 name/role tag
        # beside a character — is story content that a commercial release does letter:
        # job-3 p159's 【給仕 キーマ】 is "[SERVER: KEEMA]" in the Ichigo edition while
        # we produced nothing. `recover_captions` asks the configured model to READ the
        # crop (manga-ocr is the component that fails on this stylised text) and accepts
        # only an in-world caption; credits, colophon, titles and logos stay untouched,
        # and every failure path leaves the page exactly as it was.
        caption_pt = caption_ct = 0
        if lang == "ja" and caption_vlm and not translate_horizontal:
            blocks, caption_pt, caption_ct = recover_captions(
                image, blocks, model=model, api_key=api_key, base_url=base_url,
                timeout=llm_timeout,
            )
    ...
    shaped: set[int] = set()
    # Per-block balloon interiors measured in a LOCAL window on the ORIGINAL page, for
    # blocks whose page-wide region came back merged with a neighbour's (see
    # `_own_balloon_region` / `balloon_local`). id(block) -> page-sized bool mask.
    shape_masks: dict = {}
    # BGR view of the same page: the vendored extractor was measured against BGR input,
    # so this keeps the shipped path identical to the measured one.
    orig_bgr = balloon_local.to_bgr(image_np)
    ...
    if lang in ("ko", "zh"):
        ...
        for b in blocks:
            if b.orientation == "furigana":
                erase.extend(_erase_rects_checked(gray_page, b.bbox, page_w, page_h))
                continue
            ...
            # ---- LOCAL BALLOON (v0.28.0) ----------------------------------------
            # Same rule as the ja path below, for webtoon/manhua: when the region
            # resolved above swallows a SIBLING's own text box, two balloons have been
            # read as one interior (the `merged` class). Re-derive THIS block's balloon
            # from a local window on the original page and use it. Only for a region that
            # came from a real bubble / speech box — a drawn SFX keeps its own box and a
            # caption strip keeps class E's deliberate widening.
            if raw is not None:
                _local = _own_balloon_region(orig_bgr, b, region,
                                             _sibling_boxes(blocks, b), page_w, page_h)
                if _local is not None:
                    shape_masks[id(b)] = _local[0]
                    fitlog.record_region(b.bbox, "local-balloon", _local[1],
                                         {"was": [int(v) for v in region],
                                          "block_box": [int(v) for v in b.bbox],
                                          "lang": lang,
                                          "own_cover": round(
                                              balloon_local.own_cover(_local[0], b.bbox), 3)})
                    region = _local[1]
                    shaped.add(id(b))
            ...
            # class D: a box that lies mostly off the page must never be lettered into
            # its off-page part (job-2 p56 left a black band where the English should be)
            _reg = _region_on_page(region, page_w, page_h)
            if _reg is None:
                _warn("OFF-PAGE REGION: block box %s has no usable area on a %dx%d page — "
                      "left as-is (not erased, not lettered)",
                      tuple(int(v) for v in b.bbox), page_w, page_h)
                continue
            region = _reg
        ...
        for b in blocks:
            ...
            if b.orientation == "horizontal":
                ...
                # ---- LOCAL BALLOON (v0.28.0) ------------------------------------
                # When the region resolved above SWALLOWS a sibling's own text box, two
                # balloons have been read as one interior and the English would be fitted
                # across both outlines (the `merged` class: 1,199 of 2,606 blocks on the
                # v0.27.41 whole-library audit). Re-derive THIS block's balloon from a
                # local window on the original page — the vendored GPL-3.0 extractor, see
                # `balloon_local` — and use it, mask and box both. Measured before porting:
                # the local extraction covers 96.9% of the block's own box and 3.0% of a
                # sibling's, against 100% for the shared region we resolve here.
                # `caption-strip` is excluded on purpose: free-floating text has no
                # outline to find, and class E ("narration may widen across art") is
                # deliberate — the extractor can only ever TIGHTEN a region.
                _local = (None if _src == "caption-strip" else
                          _own_balloon_region(orig_bgr, b, region,
                                              _sibling_boxes(blocks, b), page_w, page_h))
                if _local is not None:
                    shape_masks[id(b)] = _local[0]
                    fitlog.record_region(b.bbox, "local-balloon", _local[1],
                    ...
                    _src = "local-balloon"
                    shaped.add(id(b))
                # A region that spans a SIBLING's own text box is a merged interior (two
                # overlapping balloons flooded as one): clip it back to this block's balloon
                # so the lettering cannot be drawn across the shared outline.
                _clip_sib = _sibling_boxes(blocks, b)
                _clipped = _clip_region_off_siblings(region, b.bbox, _clip_sib)
                if _clipped is not None:
                    fitlog.record_region(b.bbox, _src + "+sibling-clip", _clipped,
                                         {"was": [int(v) for v in region],
                                          "block_box": [int(v) for v in b.bbox],
                                          "siblings": len(_clip_sib)})
                    region = _clipped
                    if _src != "detector-bubble":
                        shaped.add(id(b))
            _reg = _region_on_page(region, page_w, page_h)
            if _reg is None:
                _warn("OFF-PAGE REGION: block box %s has no usable area on a %dx%d page — "
        ...
        for b in blocks:
            ...
            if is_free_floating(gray, b.bbox):
                continue
            _cont = _region_on_page(find_container(gray, b.bbox) or b.bbox, page_w, page_h)
            if _cont is None:
                _warn("OFF-PAGE REGION: block box %s has no usable area on a %dx%d page — "
                      "left as-is (not erased, not lettered)",
                      tuple(int(v) for v in b.bbox), page_w, page_h)
                continue
            targets.append((b, _cont))
            erase.extend(_erase_rects_checked(gray_page, b.bbox, page_w, page_h))
    ...
    if erase:
        emit("inpaint")
        # Record the erase footprint alongside the lettering sizes: a large erase with
        # tiny lettering IS the blank-area (class C) symptom, and it is only diagnosable
        # when both halves are in the log.
        if fitlog.enabled():
            # Wrapped because instrumentation must NEVER be able to break a render:
            # an earlier version of this call referenced an out-of-scope local and
            # raised UnboundLocalError mid-pipeline (caught by the erase-wiring tests).
            try:
                fitlog.record_erase(
                    (0, 0, int(image.width), int(image.height)),
                    sum(max(0, int(r[2])) * max(0, int(r[3])) for r in erase),
                    len(erase))
            except Exception:
                pass
        if gpu_worker_url:
    ...
    # ---- class C: recover regions far too small for their text ----------------
    # A hand-lettered line can be detected — and region-resolved — as a thin strip, so
    # the English lands at ~8px over a large blank area. Letter into at least the
    # footprint we erased for that block (`_recover_thin_region`).
    _obs = [bb for bb, _r in targets]
    for _i, (_b, _reg) in enumerate(targets):
        if _reg is None:
            continue
        try:
            _new = _recover_thin_region(gray_page, _b.bbox, _reg,
                                        _b.en_text or _b.jp_text, page_w, page_h, _obs)
        except Exception:
            _new = _reg
        if _new != _reg:
            targets[_i] = (_b, _new)
        # Log EVERY thin-region decision, applied or not: "the recovery did not fire"
        # is not diagnosable from a dump that only records successes.
        if fitlog.enabled() and min(int(_reg[2]), int(_reg[3])) < 14:
            try:
                _er = _erase_rects(gray_page, _b.bbox)
                _ua = sum(max(0, int(r[2])) * max(0, int(r[3])) for r in _er)
                fitlog.record_degenerate(
                    _b.bbox, _b.en_text or "",
                    "recovered_thin_region" if _new != _reg else "thin_unrecovered",
                    {"old": list(_reg), "new": list(_new), "erase_rects": len(_er),
                     "erase_area": _ua, "gray_is_none": gray_page is None,
                     "changed": _new != _reg})
            except Exception:
                pass
    ...
    # Replay bundle (off unless /tmp/mf_replay exists): record the exact inputs of the
    # typeset stage so a typesetter change can be replayed through two builds and
    # diffed without re-translating. See `app/pipeline/replay.py`.
    if replay.enabled():
        replay.dump(image_path, inpainted, gray_page, blocks, targets, shaped, font_id)
    emit("typeset")
    result = typeset_page(inpainted, blocks, font_id=font_id, regions=regions, only=only,
                          shapes=shaped, shape_masks=shape_masks,
                          page_label=image_path.split("/")[-1],
                          page_gray=gray_page)
